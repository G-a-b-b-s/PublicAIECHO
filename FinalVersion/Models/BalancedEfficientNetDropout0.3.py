...
from torchvision.transforms import Compose, ToTensor, Normalize, Resize, RandomHorizontalFlip, RandomVerticalFlip, \
...
class CustomDataset(Dataset):
    def __init__(self, data, transform=None):
        """
        Args:
            data (list): List of dictionaries containing image file paths and labels.
...
def main():
    ...
    logs_base_model = "/net/tscratch/people/plggabcza/AIECHO/Training_logs_updated/BalancedEfficientNetKombinacja"
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(train_data)):
        ...
        for epoch in range(50):
            ...
            if val_auc > best_auc:
                best_auc = val_auc
                patience_counter = 0
                model_path = os.path.join(logs_base_model, f"Best_EfficientNet_fold{fold + 1}.pth")
    ...
    for fold in range(num_folds):
        ...
        best_model_path = os.path.join(logs_base_model, f"Best_EfficientNet_fold{fold + 1}.pth")
    ...
    sns.heatmap(conf_matrix, annot=True, fmt='d', cmap='Blues', xticklabels=["Normal", "HFrEF", "HFpEF", "HFmrEF"],
