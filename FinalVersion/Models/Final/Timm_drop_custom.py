        ...
        if isinstance(child, nn.Sequential):
            ...
            for layer in child:
                ...
                if isinstance(layer, (nn.Conv2d, nn.Linear)):
                    new_seq.append(nn.Dropout(p=p))
            setattr(module, name, nn.Sequential(*new_seq))
        else:
            add_dropout(child, p)
    return module
...
def main():
    ...
    models = {
        "ResNet18": add_dropout(timm.create_model("resnet18", pretrained=True, in_chans=30, num_classes=4)).to(device),
        "DenseNet161": add_dropout(timm.create_model("densenet161", pretrained=True, in_chans=30, num_classes=4)).to(device),
        "ResNet50d": add_dropout(timm.create_model("resnet50d", pretrained=True, in_chans=30, num_classes=4)).to(device),
        "EfficientNetB0": add_dropout(timm.create_model("efficientnet_b0", pretrained=True, in_chans=30, num_classes=4)).to(device),
        "EfficientNetB3": add_dropout(timm.create_model("efficientnet_b3", pretrained=True, in_chans=30, num_classes=4)).to(device),
        "InceptionV3": add_dropout(timm.create_model("inception_v3", pretrained=True, in_chans=30, num_classes=4)).to(device),
    }
    ...
    for model_name, model in models.items():
        logs_base_model = f"/net/tscratch/people/plggabcza/AIECHO/Training_logs_updated/Final/{model_name}_dropout_custom"
        ...
        for fold in range(num_folds):
            ...
            train_loader = DataLoader(train_ds, batch_size=8, sampler=trainSampler, num_workers=4)
