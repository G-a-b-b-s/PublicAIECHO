...
class RegionProposalModule(torch.nn.Module):
    def __init__(self, input_dim, num_regions=4):
        super().__init__()
        self.fc = torch.nn.Linear(input_dim, num_regions * 4)
...
class GMIC(torch.nn.Module):
    def __init__(self, base_model, num_classes):
        super().__init__()
        self.global_extractor = GlobalFeatureExtractor(base_model)
        self.region_proposal = RegionProposalModule(input_dim=512)
        self.local_classifier = LocalClassifier(input_dim=512, num_classes=num_classes)
        self.global_classifier = torch.nn.Linear(512, num_classes)
    ...
    def forward(self, x):
        ...
        region_proposals = self.region_proposal(global_features)
        local_output = self.local_classifier(global_features)
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Ensure img is 3D by selecting the first depth slice (or averaging along the depth axis)
        if img.ndim == 4:
            img = img[..., 0]
        elif img.ndim == 3:
            img = img[:, :, 0]
...
def main():
    ...
    logs_base_model = "/net/tscratch/people/plggabcza/AIECHO/Training_logs_updated/GMIC"
    ...
    model = GMIC(base_model, num_classes).to(device)
