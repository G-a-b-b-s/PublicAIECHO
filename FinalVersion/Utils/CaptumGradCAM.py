...
def save_heatmap_jpg(grayscale_cam, input_tensor, save_path_jpg, title=None):
    ...
    if input_image.ndim == 3 and input_image.shape[0] == 30:
        frame_idx = 15
        input_image = input_tensor.squeeze()[frame_idx].detach().cpu().numpy()
    ...
    # Apply colormap to the Grad-CAM mask
    heatmap = (grayscale_cam * 255).astype(np.uint8)
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)  # <--- This adds color

    heatmap = cv2.cvtColor(heatmap, cv2.COLOR_BGR2RGB)

    # Overlay heatmap onto input image
    overlayed = (0.4 * heatmap / 255.0 + 0.6 * input_image_rgb)
    overlayed = np.clip(overlayed, 0, 1)

    cam_image = (overlayed * 255).astype(np.uint8)
...
def main():
    ...
    output_root = "/net/tscratch/people/plggabcza/captum_gradcam_results"
    ...
    for fold in range(5):
        ...
        for idx, (imgs, labels, paths, patient_ids) in enumerate(tqdm(loader)):
            ...
            attributions = cam.attribute(imgs, target=pred_class)
            grayscale_cam = torch.nn.functional.interpolate(
                attributions,
                size=imgs.shape[2:],
                mode="bilinear",
                align_corners=False
