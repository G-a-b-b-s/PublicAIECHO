...
if __name__ == '__main__':
    ...
    # Process FoldedTrain (Fold0–Fold4)
    folded_train_dir = os.path.join(base_dir, 'FoldedTrain')
    folds = [f'Fold{i}' for i in range(5)]
    classes = ['HFpEF', 'HFrEF', 'Normal', 'HFmrEF']

    for fold in folds:
        for class_name in classes:
            input_dir = os.path.join(folded_train_dir, fold, class_name)
            output_dir = os.path.join(output_base_dir, fold, class_name)
            os.makedirs(output_dir, exist_ok=True)

            for file in os.listdir(input_dir):
                if file.endswith('.mp4'):
                    video_path = os.path.join(input_dir, file)
                    results = create_3d_images_from_video(video_path, output_dir=output_dir)
                    print(f"Finished processing {file} in {fold}/{class_name}")
                    print('----------------------------')

    # Process Test set
    test_dir = os.path.join(base_dir, 'Test')
    for class_name in classes:
        input_dir = os.path.join(test_dir, class_name)
        output_dir = os.path.join(output_base_dir, 'Test', class_name)
        os.makedirs(output_dir, exist_ok=True)
        ...
        for file in os.listdir(input_dir):
            if file.endswith('.mp4'):
                video_path = os.path.join(input_dir, file)
                results = create_3d_images_from_video(video_path, output_dir=output_dir)
                print(f"Finished processing {file} in Test/{class_name}")
                print('----------------------------')
