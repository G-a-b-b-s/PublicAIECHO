...
class ModifyContrast:
    ...
    def __call__(self, tensor):
        return (((tensor - tensor.min())/(tensor.max()-tensor.min())*(self.ratio*tensor.max()) + tensor.min()).long()).float()
...
def main(fold_to_run):
    ...
    train_transforms = Compose([
        ...
        #RandomApply([GaussianBlur(kernel_size=5, sigma = 0.05)], p=0.3),
    ...
    for model_name, model in models.items():
        ...
        logs_base_model = "/net/tscratch/people/plgztabor/ECHO/logs1/"
